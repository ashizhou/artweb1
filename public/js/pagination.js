function pagination(data,callback){
	...
	if(!document.getElementById('pageStyle')){
		...
		style.id = 'pageStyle'
		style.innerHTML='.pagination{text-align:center;margin-top:100px}.pagination a,.pagination span{margin:0 2px;padding:4px 8px;color:#30a07e;background:#fff;text-decoration:none;border:1px solid #ddd;border-radius:4px;user-select:none;cursor:pointer}.pagination a:hover,.pagination span:hover{color:#fff;background:#30a07e}.pagination .active{color:#fff;background:#30a07e;cursor:default;}.pagination input{width:40px;padding:7px 0;border:none;outline:0;border:1px solid #ddd;border-radius:4px;text-align:center;margin:0 5px}.pagination i{font-style: normal;margin:0 5px;color:#999}.pagination input:focus{border:1px solid #30a07e}'
	...
	function pageAction(dataPage){
		...
		if (data.jumpBtn && data.totalPage >= 1) {
			i_html += '鍓嶅線<input id="pageInput" type="text" />椤� <span id="inputGo">确定</span>'
		}
