...
var artlikes = `create table if not exists likes(
    id INT(200) NOT NULL AUTO_INCREMENT,
    name VARCHAR(100) NOT NULL,
    artid INT(200) NOT NULL,
    PRIMARY KEY (id),
...
let poseLikes = function(id){
    let _sql = `delete from artlikes where id = "${id}"`
    return query(_sql);
}
...
let findArtByPage=function(page){
    let _sql = ` select * FROM art Limit ${(page-1)*15},15`
    return query( _sql)
  }
...
let updateArtPv = function(value){
    let _sql = "update art set pv=? where id=?"
    return query(_sql,value);
}
...
let findCommentByPage = function(page,artid)  {
    let _sql = `select * from artcomment where id=${artid} order by id desc limit ${(page-1)*10},10;`
    return query(_sql);
  }
